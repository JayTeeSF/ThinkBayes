...
module ThinkBayes
  ...
  def debug_puts debug=DEBUG, &block
    if debug_print(&block) # pass block
      puts ""
    end
  end
  ...
  def help
    %Q^
      #prompt> irb -r ./think_bayes.rb
      #irb> pmf = ThinkBayes::ProbabilityMassFunction.new
      #irb> (1..6).to_a.each {|x| pmf.set(x, 1.0/6.0) }
      #
      #irb> pmf = ThinkBayes::ProbabilityMassFunction.new
      #irb> %w{ this is it. this is the end }.each{|word| pmf.incr(word, 1) }
      #irb> pmf.probability('the') #<-- hmm!! how can this yield 1
      #irb> pmf.normalize
      #irb> pmf.probability('the') #<-- ah!
      #
      #irb> pmf = ThinkBayes::ProbabilityMassFunction.new
      #irb> pmf.set('bowl 1', 0.5)
      #irb> pmf.set('bowl 2', 0.5)
      #irb> pmf.probability('bowl 1')
      #
      #likelihood (given new info about vanilla)
      #irb> pmf.multiply('bowl 1', 0.75)
      #irb> pmf.multiply('bowl 2', 0.5)
      #
      #irb> pmf.normalize
      #irb> pmf.probability('bowl 1')
    ^
  ...
  def odds(_probability)
    # Computes odds for a given probability.
    # Example: _probability=0.75 means 75 for and 25 against, or 3:1 odds in favor.
    # Note: when _probability=1, the formula for odds divides by zero, which is
    # normally undefined.  But I think it is reasonable to define Odds(1)
    # to be infinity, so that's what this function does.
    # _probability: float 0-1
    # Returns: float odds

    return nil if _probability == 1
    Float(_probability) / (1 - _probability)
  end

  def probability(_odds)
    # Computes the probability corresponding to given odds.
    # Example: o=2 means 2:1 odds in favor, or 2/3 probability
    # _odds: float odds, strictly positive
    # Returns: float probability

    return Float(_odds) / (_odds + 1)
  end
  ...
  class Interpolator
    ...
    def lookup(x)
      ...
      return _bisect(x, xs, ys)
    end

    def reverse(y)
      #"""Looks up y and returns the corresponding value of x."""
      return _bisect(y, ys, xs)
    end
  ...
  class DictWrapper
    ...
    def initialize(values={}, name='')
      ...
      if values.respond_to?(:empty?)
        return if values.empty?
      elsif values.respond_to?(:first)
        return unless !!values.first
      else
        warn "unsupported values object"
        return
      end
    ...
    #     :init_hypothesis,
      ...
      init_methods.each do |method_name|
        # puts "calling: #{method_name}(#{values})"
        if respond_to?(method_name) && send(method_name, values)
          break
        end
      end
    ...
    def init_sequence(values)
      ...
      if values.respond_to?(:each_char)
        iterator = :each_char
      end

      values.send(iterator) do |value|
        set(value, 1)
      end
    end
    ...
    def init_mapping(values)
      ...
      values.each_pair do |value, prob|
        set(value, prob)
      end
    end

    def init_hypothesis(values)
      return unless values.respond_to?(:each)

      values.each do |hypo|
        set(hypo.name, hypo.value)
      end
    end
    ...
    def exp(m=nil)
      ...
      unless log
        warn("Pmf/Hist not under a log transform")
      end
      @log = false

      m = max_like() unless m

      d.each_pair do |value, prob|
        set(x, Math.exp(p - m))
      end
    end
    ...
    def init_failure(values)
      # Raises an error.
      warn 'None of the initialization methods worked.'
    end
    ...
    def incr(x, term=1)
      ...
      @d[x] = d.fetch(x, 0) + term
    end

    def multiply(x, likelihood_factor)
      #Scales the freq/prob associated with the value x.
      # Args:
      #     x: number value
      #     likelihood_factor: how much to multiply by

      @d[x] = d.fetch(x, 0) * likelihood_factor
    end
...
# 
     ...
     alias_method :values, :hypotheses
...
# 
  ...
  class ProbabilityMassFunction < DictWrapper
    #Represents a probability mass function.
    #Values can be any hashable type; probabilities are floating-point.
    #Pmfs are not necessarily normalized.

    def probability(x, default=0)
    ...
    def probabilities(xs=hypotheses)
      # Gets probabilities for a sequence of values.
      return xs.map{|x| probability(x) }
    end
    ...
    def normalize(fraction=1.0)
      ...
      warn("Pmf is under a log transform") if @log

      _total = total
      if 0.0 == _total
        warn('total probability is zero.')
        warn('Normalize: total probability is zero.')
        return _total
      end

      factor = Float(fraction) / _total
      hypotheses.each {|x| @d[x] *= factor }
    ...
    # FIXME when to clear this cache!?
    def numeric_hypotheses?
      unless @numeric_hypotheses
        @numeric_hypotheses = hypotheses.all?{|h| h.is_a?(Numeric) }
      end
      @numeric_hypotheses
    end

    def variance(mu=mean)
    ...
    def MaximumLikelihood
      ...
      # if numeric_hypotheses? <-- in theory "val" is a non-numeric hypothesis
      d.each_pair do |val, prob|
       if prob >= max_prob
         max_val = val
       end
      end
  ...
  class Suite < ProbabilityMassFunction
    ...
    def update(data)
      ...
      hypotheses.each do |hypo|
        ThinkBayes.debug_print { "updating hypothesis #{hypo}, based on evidence of (#{data})" }
        like = likelihood(data, hypo)
        ThinkBayes.debug_puts { " resulting in new likelihood of #{like}" }
        multiply(hypo, like)
        normalize
      end
    end
